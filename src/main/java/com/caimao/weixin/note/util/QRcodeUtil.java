...
public class QRcodeUtil {
	...
	public static void main(String[] args) throws WriterException {
		try {
			// String url =
			// "//http://gupiao.caimao.com/weixin/note/reader/read/" +
			// "reader.getReader_note_id()";
			// getLogoQRCode("http://gupiao.caimao.com/weixin/note/reader/read/1011",
			// null, "跳转到作者笔记页面");
			//String string = "http://wewer.werwe/we/1";
	...
	public static String getLogoQRCode(String qrUrl, String path) {
		try {
			...
			BufferedImage bim = zp.getQR_CODEBufferedImage(qrUrl, BarcodeFormat.QR_CODE, 400, 400,
					zp.getDecodeHintType());
			// return zp.addLogo_QRCode(bim, new File(filePath), new
			// LogoConfig(), productName);
			return zp.getQRCode(bim, path, qrUrl.substring(qrUrl.lastIndexOf("/") + 1));
		} catch (Exception e) {
			LOGGER.info("生成带logo的二维码图片错误");
			e.printStackTrace();
		}
		return null;
	}
	...
	/** * 返回二维码字符串 */
	public String getQRCode(BufferedImage bim, String path, String noteId) {
		bim.flush();
		...
		try {
			...
			// 前端用 <img src="data:image/jpg;base64,${imageBase64QRCode}"/>
			// 其中${imageBase64QRCode}对应二维码的imageBase64字符串
			ImageIO.write(bim, "jpg", new File(path + File.separator + noteId + ".jpg"));
		} catch (IOException e) {
			LOGGER.info("生成二维码字符串错误");
			e.printStackTrace();
		}
	...
	public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName) {
		try {
			 ...
			 * g.setColor(logoConfig.getBorderColor()); g.drawRect(x, y,
			...
			// 前端用 <img src="data:image/jpg;base64,${imageBase64QRCode}"/>
			// 其中${imageBase64QRCode}对应二维码的imageBase64字符串
			//ImageIO.write(image, "jpg", new File("D:" + File.separator + new Date().getTime() + "_test.jpg")); // TODO
