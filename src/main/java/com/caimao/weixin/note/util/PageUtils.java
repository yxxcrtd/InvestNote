...
public class PageUtils {
	...
	private HashMap<String, String> config = new HashMap<String, String>() {
        ...
        put("first", "第一页");
        put("last", "最后一页");
        put("footer", "<li class=\"\"><span>共 %TOTAL_PAGE% 页</span></li>");
        /*put("theme", "%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %FOOTER%");*/
        put("theme", "%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%");
    }};
    ...
    public String show(){
        ...
        String upPage = upRow > 0?"<li><a class='pre' href=\"" + this.url + upRow + "\">" + this.config.get("prev") + "</a></li>":"<li><span class='none'>"+this.config.get("prev")+"</span></li>";

        //下一页
        Integer downRow = this.nowPage + 1;
        String downPage = downRow <= this.totalPages?"<li><a class='next' href=\"" + this.url + downRow + "\">" + this.config.get("next") + "</a></li>":"<li><span class='none'>"+this.config.get("next")+"</span></li>";

        //第一页
        String firstPage = "";
        if(this.totalPages > this.rollPage && (this.nowPage - nowCoolPage) >= 1){
            firstPage = "<li><a href=\"" + this.url + "1\">" + this.config.get("first") + "</a></li>";
        }

        //最后一页
        String lastPage = "";
        if(this.totalPages > this.rollPage && (this.nowPage + nowCoolPage) >= this.totalPages){
            lastPage = "<li><a href=\"" + this.url + this.totalPages + "\">" + this.config.get("last") + "</a></li>";
        }
        ...
        return "<ul class=\"pagination\">" + theme + "</ul>";
    }
}