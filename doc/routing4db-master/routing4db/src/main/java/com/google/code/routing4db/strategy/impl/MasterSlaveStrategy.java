...
/**
 * 实现 Master-Slave路由策略。其中Master是默认数据源，Slave根据默认数据源指定。
 * 写Master， 读Slave。可通过把Master的key放到Slave中实现写Master，读Master-Slave多个Slave对象，则随机选择一个。
 */
public class MasterSlaveStrategy extends AbstractRoutingStrategy implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        if (null == readMethodPatterns || null == dataSourceKeyMap) {
